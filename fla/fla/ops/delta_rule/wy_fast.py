...
@triton.jit(do_not_specialize=['T'])
def prepare_wy_repr_non_local_bwd_kernel(
    k,
    v,
    beta,
    A,
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        b_v = tl.load(p_v, boundary_check=(0, 1))
        b_v_beta = (b_v * b_beta[:, None]).to(b_v.dtype)
        b_du = tl.load(p_du, boundary_check=(0, 1)).to(b_v.dtype)
        b_dA += tl.dot(b_du, tl.trans(b_v_beta), allow_tf32=False)
        b_dv_beta = tl.dot(b_A, b_du, allow_tf32=False)
        b_dv = b_dv_beta * b_beta[:, None]
        b_dbeta += tl.sum(b_dv_beta * b_v, 1)
...
def prepare_wy_repr_non_local_bwd(
    k: torch.Tensor,
    v: torch.Tensor,
    beta: torch.Tensor,
    A: torch.Tensor,
    ...
    dv = torch.empty_like(v)
    dbeta = torch.empty_like(beta)
    prepare_wy_repr_non_local_bwd_kernel[(NT, B * H)](
        k,
        v,
        beta,
        A,
