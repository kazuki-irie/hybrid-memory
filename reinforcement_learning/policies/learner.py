...
class Learner:
    ...
    def train(self):
        ...
        wandb.init(project=self.FLAGS.wandb_name, config=self.config_seq)
        ...
        while self._n_env_steps_total < self.n_env_steps_total:
            ...
            if current_num_iters % self.config_env.eval_interval == 0:
                self.log()

            # self.agent.eval()  # crucial to keep dropout on
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                env_steps = self.collect_rollouts(num_rollouts=1)  # this updates self._n_env_steps_total
    ...
    def log(self):
        ...
        wandb.log({"env_steps": self._n_env_steps_total,
                   "return": np.mean(returns_eval),
                   "success": np.mean(success_rate_eval),
                   "length": np.mean(total_steps_eval)})
