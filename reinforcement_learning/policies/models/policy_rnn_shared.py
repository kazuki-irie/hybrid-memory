...
class ModelFreeOffPolicy_Shared_RNN(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.drop_obs_embedding = nn.Dropout(dropout)
        # self.drop_seq_out = nn.Dropout(dropout)
    ...
    def get_hidden_states(
        self, prev_actions, rewards, observs, initial_internal_state=None, get_state=False,
    ):
        ...
        # dropout
        input_s = self.drop_obs_embedding(input_s)
        inputs = torch.cat((input_a, input_r, input_s), dim=-1)
        # feed into RNN: output (T+1, B, hidden_size)
        if initial_internal_state is None and not get_state:  # training
    ...
    def forward(self, actions, rewards, observs, dones, masks):
        ...
        # dropout
        # hidden_states = self.drop_seq_out(hidden_states)
